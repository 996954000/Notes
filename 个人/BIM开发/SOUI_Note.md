...
### 1.6.9 targetver.h 定义 Windows 版本
```c++
#pragma once
...
### 1.6.10 SOUITemplate.cpp 应用程序入口 找 _tWinMain 为程序入口
```c++
#include "stdafx.h"
...
### 1.6.11 CMainWnd 为了方便演示全写在头文件里了
```c++
#pragma once
...
### 1.6.12 添加主窗口运行代码 打开主窗口 CMainWnd
```c++
            CMainWnd wndMain;
...
## 1.7 error C2065: “SApplication”: 未声明的标识符
### 1.7.1 根本原因：C++20 严格模式不兼容 SoUI
SoUI 的头文件存在**循环包含**：
...
## 1.8 无法解析的外部符号 main
### 1.8.1 SoUI 是 Windows 窗口程序，入口函数不是 main，而是 WinMain
...
### 2.4.4 `IResProvider`：资源提供者（在程序中从哪里加载）
...
--------------------------------------------------------------------------
## 2.7 SOUI：TreeView + Adapter 用法笔记
...
### 2.10.1 DirectUI 的性能问题

DirectUI 的底层：**整个窗口只有一个 HWND，框架自己遍历控件树做光栅化**。每一帧都要经历：
...
**只要 UI 里有明显的 Alpha 混合需求（半透明、圆角、阴影、现代扁平风格），优先用 Skia**。 `COM_RENDER_GDI` 的加载代码只要换成 `render-skia.dll` 就能切换。

---

### 2.10.3 脏矩形（Dirty Rect）+ 剪裁区（Clip）
...
SOUI 默认**全部使用内存 DC 离屏绘制**，绘制完再一次性贴到屏幕，所以默认就不会闪烁。
...
### 2.10.6 `bkgndBlend="0"` —— 跳过父窗口背景混合

- 默认 `bkgndBlend="1"`：控件重绘时，**会先请求父窗口把自己下面的背景画一遍**（为了保证半透明合成正确）
- 设为 `0`：控件不请求父背景，**自己直接画**（像是深度剔除）
